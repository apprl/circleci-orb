...
function DeleteIPSet(){
	...
	if [[ -n "$IPLIST" ]]; then
        UPDATESET=$(aws waf update-ip-set --ip-set-id ${IPSETID} --change-token ${CHANGETOKEN} --updates "$(echo "$IPLIST"| jq '[.[] | { IPSetDescriptor: {Type: .[].Type, Value: .[].Value}, Action: "DELETE"} ] | unique' --slurp)" 2>&1) # | jq .)
